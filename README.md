...
- **Room picture:** `docs/assets/room.svg` restores the exact room from before the Dell timeline update, including its face, shirt, shorts, chair and sausage legs. The existing Tahoe wallpaper stays on the MacBook; only the Dell screen uses `room-dell-legs.webp`, showing AIMVS on the left and VS Code on the right. The shorter legs and later full-room rebuild are not applied. Run `python3 scripts/build-room-image.py` after a screen-asset change, then rebuild; never replace the rest of the photo as part of a screen edit.
