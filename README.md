...
- **Room picture:** `docs/assets/room.svg` combines the corrected face, work screen, OBS screen, laptop wallpaper and small Canon camera patch with the original `room.webp`. The Dell shows an AIMVS timeline from my YC demo on the left and my VS Code diff view on the right. `room-dell-legs.webp` supplies that screen and the shorter sausage legs; the existing face, clothes and other image layers stay intact. After replacing `room-screens.webp`, `room-details.webp`, `room-camera.webp`, `room-dell-legs.webp` or adjusting the region outlines, run `python3 scripts/build-room-image.py`, then rebuild. See [source notes](SOURCES.md) for references and privacy edits.
