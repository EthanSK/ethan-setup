...
Place a screen hotspot from the compositor's polygon in `scripts/build-room-image.py` or a crop of the matching insert asset; a Quick Look thumbnail of `room.svg` renders as a stretched square, so its pixel positions do not map to the 1586 × 992 photo. The VS Code hotspot sits on the right half of the Dell polygon, separate from the Dell product and Codex hotspots.

Walkthrough next/previous buttons must bring the selected slide tab back into view. Preventing focus scroll leaves mobile readers at the bottom of the next slide, hiding its introduction; test navigation after scrolling through the longest slide, not only from the top.
...
Ethan rejected the full-room rebuild and selected the earlier `c220035` reference after comparing saved pictures. Preserve that complete SVG composition, including its existing MacBook wallpaper and portrait layers, and add only the later Dell screen through its existing polygon. This selected version predates the Canon patch; do not reapply it, the shortened-leg region or the later full-room reconstruction. A lossless render comparison proves that all pixels outside the Dell screen remain identical. An image generator's “preserve” prompt does not guarantee pixel stability; keep display-only changes constrained in the compositor.

Image changes must use matching hotspot coordinates and mobile framing. Restoring the earlier photo also restores its anchors and portrait crop; keyboard panning still uses the same bounds as dragging. Keep the social card on the same active composition. For non-browser SVG exports, Sharp requires xlink image references and PNG decoding of embedded WebPs in this runtime; inspect the result and compare it with the browser before exporting a social card.

## Directory relevance

Ethan's “dock” described a rough location, not permission to recreate the complete installed-app Dock over the photo. He rejected that overlay. Keep the existing Software rail, directory and separate Codex/OBS room hotspots; do not reintroduce the app grid or its long detour through dialog navigation.

Rank every directory section roughly by current use, with public and private-safe personal skills interleaved so publication status does not bury frequently used workflows. Preserve local app ordering across canonical inventory refreshes and place newly discovered apps at the end until reviewed. macOS foreground use undercounts background mouse/dictation utilities; recent skill reads can overcount audits and undercount renamed skills. Never label the ordering as precise live telemetry or publish raw usage records.

Verify a repository's actual purpose before classifying it. Honest Comments analyzes YouTube criticism; it is not about code comments. Agent Swarm Management is a Mac app and Agent Bridge is a project, so neither belongs among skills. Keep their existing public project links, with no duplicate skill cards.
...
Distance-only pinch changes magnification but loses the photo point between the fingers. Anchor that point in room coordinates at the start, then combine the new separation and moving midpoint; share the camera scale calculation with rendering and one-finger dragging. Mouse, pen and touch presses starting on room item labels must join the gesture, while toolbar buttons and links keep their normal input. Restricting that exception to touch prevented mouse dragging and could turn its release into a background zoom. Suppress release clicks from a drag or the whole multi-touch gesture until a new press, and preserve ordinary and keyboard clicks. Rebase a remaining finger from the current view, not the camera matrix from the preceding render frame; cancel when resizing invalidates screen coordinates.
